...
register_shutdown_function(function() {
    ...
    if ($err && in_array($err['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR])) {
        ...
        @file_put_contents($debugDir . '/phar_sales_fatal.log', $msg, FILE_APPEND | LOCK_EX);
        // Capture any buffered stray output for debugging
        if (ob_get_length() !== false) {
            $buf = ob_get_contents();
            @ob_end_clean();
            if (trim($buf) !== '') @file_put_contents($debugDir . '/phar_sales_stray_output.log', "[".date('c')."]\n".$buf."\n", FILE_APPEND | LOCK_EX);
        }
...
try {
    ...
    @file_put_contents($debugDir . '/phar_sales_request.log', "[".date('c')."] METHOD: {$_SERVER['REQUEST_METHOD']} RAW_BODY:\n" . $rawBody . "\n_POST:\n" . $postSnapshot . "\n\n", FILE_APPEND | LOCK_EX);
...
$respLog = __DIR__ . '/../../tmp/phar_sales_response.log';
@file_put_contents($respLog, "[".date('c')."] RESPONSE_JSON:\n" . json_encode($response) . "\n", FILE_APPEND | LOCK_EX);

if (!empty($buffer) && trim($buffer) !== '') {
    $logFile = __DIR__ . '/../../tmp/phar_sales_debug.log';
