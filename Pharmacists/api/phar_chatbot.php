...
function find_medicine(mysqli $conn, string $query): ?array {
    ...
    foreach ($terms as $term) {
        $where[] = "(LOWER(name) LIKE ? OR LOWER(barcode) LIKE ? OR LOWER(type) LIKE ? OR LOWER(description) LIKE ?)";
        $like = '%'.$term.'%';
...
function expiry(mysqli $conn, string $q): array {
    ...
    $sql  = $includeExpired
        ? "SELECT name, quantity, type, expiry_date FROM medicines WHERE expiry_date IS NOT NULL AND expiry_date <= ? ORDER BY expiry_date ASC LIMIT 30"
        : "SELECT name, quantity, type, expiry_date FROM medicines WHERE expiry_date IS NOT NULL AND expiry_date BETWEEN CURDATE() AND ? ORDER BY expiry_date ASC LIMIT 30";
...
function medicine_lookup(mysqli $conn, string $query): array {
    ...
    return response_pack('medicine_lookup', $response, [
        'cards' => [
            ['label' => 'Current stock',   'value' => number_format((int)$med['quantity']), 'meta' => $med['type'] ?: 'No type'],
...
function purchases(mysqli $conn, string $q): array {
    ...
    return response_pack('purchases', count($data).' purchase record(s)'.($status ? " with status '{$status}'" : '').'.', [
        ...
        'actions' => [['label' => 'Open Purchases', 'href' => 'purchases.php', 'icon' => 'bi-bag-check']],
    ]);
}
