...
// Pharmacists can view the shared system settings, but only administrators can change them.
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    http_response_code(403);
    echo json_encode(['error' => 'Only administrators can update system settings.', 'success' => false]);
    exit();
}
...
// Helper: Get setting value (system-wide)
function getSetting($conn, $key, $default = null, $userId) {
    try {
...
// Helper: Set setting value (system-wide)
function setSetting($conn, $key, $value, $userId) {
    try {
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        $settings = [
            ...
            'notif_purchase' => getSetting($conn, 'notif_purchase', '1', $userId),
            'currency_symbol' => getSetting($conn, 'currency_symbol', '₱', $userId),
    ...
    try {
        ...
        try {
            ...
            foreach ($input as $key => $value) {
                ...
                if (!$result) {
                    $failedSettings[] = $key;
                    error_log("Failed to save system-wide setting: $key = $value");
                } else {
                    $successCount++;
                }
            ...
            error_log("Successfully saved $successCount system-wide settings by user $userId");

            http_response_code(200);
            echo json_encode([
                'success' => true, 
                'message' => 'Settings saved successfully. Changes applied system-wide.',
                'saved_count' => $successCount,
                'global_updated' => array_keys($input)
            ]);
