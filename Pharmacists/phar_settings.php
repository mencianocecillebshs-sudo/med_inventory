...
<body>
    ...
    <script src="assets/js/phar_settings.js"></script>
    <script>
        // Settings are shared system-wide. Pharmacists may review them but cannot alter them.
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('#settings-form input, #settings-form select, #settings-form textarea, #settings-form button').forEach((element) => element.disabled = true);
            document.querySelectorAll('#settings-form button, #confirm-save-btn, #cancel-changes').forEach((element) => element.hidden = true);
            document.getElementById('settings-form')?.insertAdjacentHTML('afterbegin', '<div class="alert alert-info"><i class="bi bi-eye me-1"></i>Settings are managed by an administrator and are shown here for reference.</div>');
        });
    </script>
</body>
</html>
