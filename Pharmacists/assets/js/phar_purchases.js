document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Medicine / Other Products picker mode ──────────────────────────
    // Filters what appears in every row's combobox, and relabels the
    // section so it's clear which kind of item is being added.
    let currentItemTypeFilter = ''; // '' = medicine (default), 'non-medicine' = other products

    function applyItemTypeMode(mode) {
        currentItemTypeFilter = mode === 'non-medicine' ? 'non-medicine' : '';
        const medBtn = document.getElementById('pick-medicine-items-btn');
        const otherBtn = document.getElementById('pick-other-products-btn');
        const sectionTitle = document.getElementById('med-section-title');
        const colHeader = document.getElementById('med-picker-col-header');
        const addBtnLabel = document.getElementById('add-med-row-btn-label');

        if (currentItemTypeFilter === 'non-medicine') {
            medBtn?.classList.remove('active');
            otherBtn?.classList.add('active');
            if (sectionTitle) sectionTitle.innerHTML = '<i class="bi bi-box-seam"></i> Non Medicine';
            if (colHeader) colHeader.innerHTML = '<i class="bi bi-search me-1"></i> Non Medicine <span class="required-indicator">*</span>';
            if (addBtnLabel) addBtnLabel.textContent = 'Add Product';
        } else {
            otherBtn?.classList.remove('active');
            medBtn?.classList.add('active');
            if (sectionTitle) sectionTitle.innerHTML = '<i class="bi bi-capsule"></i> Medicine Items';
            if (colHeader) colHeader.innerHTML = '<i class="bi bi-search me-1"></i> Medicine <span class="required-indicator">*</span>';
            if (addBtnLabel) addBtnLabel.textContent = 'Add Medicine';
        }
    }

    document.getElementById('pick-medicine-items-btn')?.addEventListener('click', () => applyItemTypeMode('medicine'));
    document.getElementById('pick-other-products-btn')?.addEventListener('click', () => applyItemTypeMode('non-medicine'));

    // ── Load medicines ───────────────────────────────────────────────
    async function loadMedicines() {
        try {
            const res = await fetch('api/phar_medicines.php?limit=9999');
    ...
    function wireMedCombobox(tr) {
        ...
        function renderList(filter) {
            ...
            const pool = currentItemTypeFilter
                ? medicines.filter(m => (m.item_type || 'medicine') === currentItemTypeFilter)
                : medicines.filter(m => (m.item_type || 'medicine') === 'medicine');
            const filtered = q
                ? pool.filter(m => m.name.toLowerCase().includes(q))
                : [...pool];

            if (filtered.length === 0) {
                list.innerHTML = '<div class="med-dd-empty">No ' + (currentItemTypeFilter === 'non-medicine' ? 'products' : 'medicines') + ' found.</div>';
                return;
            }
    ...
    function loadPurchases(query = '', medicineId = '', status = '') {
        ...
        let url = `api/phar_purchases.php?per_page=9999`;
            ...
            .then(res => {
                ...
                purchases.forEach(p => {
                    ...
                    if (p.status === 'pending') {
                        ...
                        // still pending — once it's filled/cancelled, phar_purchases.php rejects
                        // any update, so there's no point offering the button here.
    ...
    // pharmacist there with the purchase pre-selected, instead of finalizing it here.
    function handleRingUp(purchaseId) {
        window.location.href = `phar_sales.php?purchase_id=${encodeURIComponent(purchaseId)}`;
    }

    // ── Receipt / Print ──────────────────────────────────────────────
    async function printPurchase(id) {
        try {
            const res = await fetch(`api/phar_purchases.php?id=${id}`);
    ...
    document.getElementById('newPurchaseForm')?.addEventListener('submit', async (e) => {
        ...
        for (const tr of medRows) {
            ...
            try {
                const res = await fetch('api/phar_purchases.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    // Throttled to at most once every 3s: if api/phar_realtime.php ever fires
    ...
    if (typeof EventSource !== 'undefined') {
        const source = new EventSource('api/phar_realtime.php');
