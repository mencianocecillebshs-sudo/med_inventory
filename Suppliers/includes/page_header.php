<?php
if (defined('SUPPLIER_PAGE_HEADER_RENDERED')) {
    return;
}
define('SUPPLIER_PAGE_HEADER_RENDERED', true);

$scriptFile = basename(str_replace('\\', '/', $_SERVER['SCRIPT_NAME'] ?? ''));
$pageKey = preg_replace('/\.php$/', '', $scriptFile);
$plainKey = preg_replace('/^sup_/', '', $pageKey);

$pageMeta = [
    'dashboard' => ['Dashboard', 'bi-speedometer2'],
    'medicine' => ['My Inventory', 'bi-box-seam'],
    ...
    'help' => ['Help & Support', 'bi-question-circle'],
];

$navSections = [
    'Supplier Workspace' => [
        ['sup_dashboard.php', 'dashboard', 'bi-speedometer2', 'Dashboard'],
        ['sup_medicine.php', 'medicine', 'bi-box-seam', 'My Inventory'],
        ['sup_orders.php', 'orders', 'bi-bag-check', 'Orders'],
        ['sup_sales.php', 'sales', 'bi-cart-check', 'Sales'],
        ['sup_transactions.php', 'transactions', 'bi-receipt', 'Transactions'],
    ],
    'Private Insights' => [
        ['sup_analytics.php', 'analytics', 'bi-graph-up-arrow', 'Analytics'],
        ['sup_reports.php', 'reports', 'bi-file-earmark-bar-graph', 'Reports'],
    ],
    'System' => [
        ['sup_notifications.php', 'notifications', 'bi-bell', 'Notifications'],
        ['sup_chatbot.php', 'chatbot', 'bi-chat-dots', 'AI Assistant'],
        ['sup_settings.php', 'settings', 'bi-gear', 'Settings'],
        ['sup_help.php', 'help', 'bi-question-circle', 'Help & Support'],
    ],
];

[$pageTitle, $pageIcon] = $pageMeta[$plainKey] ?? [ucwords(str_replace('_', ' ', $plainKey)), 'bi-folder2-open'];
$isDashboard = $plainKey === 'dashboard';
$supplierName = isset($_SESSION['username']) ? $_SESSION['username'] : 'Supplier';
$supplierRole = isset($_SESSION['role']) ? ucfirst($_SESSION['role']) : 'Supplier';
?>

<div class="admin-breadcrumb-header supplier-breadcrumb-header" role="navigation" aria-label="Breadcrumb">
    <div class="admin-breadcrumb-shell">
        <div class="admin-breadcrumb-brand">
            ...
            <div>
                <div class="admin-breadcrumb-eyebrow">Supplier Workspace</div>
                <ol class="breadcrumb admin-breadcrumb mb-0">
                    <li class="breadcrumb-item">
                        <a href="sup_dashboard.php">
                    ...
                    <li class="breadcrumb-item">
                        <?php if (!$isDashboard) { ?>
                            <a href="sup_dashboard.php">Supplier</a>
                        <?php } else { ?>
                            <span>Supplier</span>
                        <?php } ?>
                    </li>
        ...
        <div class="admin-header-actions">
                        ...
                        <div class="admin-nav-section">
                            ...
                            <div class="admin-nav-grid">
                                <?php foreach ($items as $item) {
                                    ...
                                    $activeClass = $key === $plainKey ? ' active' : '';
                                ?>
            ...
            <div class="admin-user-chip" title="<?php echo htmlspecialchars($supplierName . ' - ' . $supplierRole); ?>">
...
<style>
    ...
    .admin-breadcrumb-shell {
        ...
        border-radius: 14px;
        background: linear-gradient(135deg, rgba(27, 94, 63, 0.08), rgba(15, 63, 40, 0.03)), #ffffff;
    ...
    body.dark-mode .admin-breadcrumb-shell {
        ...
        background: linear-gradient(135deg, rgba(46, 204, 113, 0.12), rgba(15, 63, 40, 0.12)), #111827;
