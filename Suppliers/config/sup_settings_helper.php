...
// Normalize currency symbol — DB may contain "?" from past encoding issues.
function normalizeCurrencySymbol($symbol) {
    $symbol = is_string($symbol) ? trim($symbol) : '';
    if ($symbol === '' || $symbol === '?' || preg_match('/^\?+$/', $symbol)) {
        return '₱';
    }
    return $symbol;
}
...
function getCurrencySymbol($conn) {
    return normalizeCurrencySymbol(getUserSetting($conn, 'currency_symbol', '₱'));
}
...
function createNotification($conn, $message, $type = '') {
    ...
    $typeMap = [
        'shortage' => 'low_stock',
        'expiry' => 'expiry',
        '' => 'prescription' // default to prescription for general notifications
    ];
    
    $settingKey = isset($typeMap[$type]) ? $typeMap[$type] : 'prescription';
