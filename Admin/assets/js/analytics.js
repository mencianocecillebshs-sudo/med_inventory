...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadTop = (s, e) => {
        ...
        fetch(`api/analytics.php?action=top_medicines&limit=10&start=${s}&end=${e}`)
            .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
            .then(res => {
    ...
    const loadLowStock = () => {
        ...
        fetch(`api/analytics.php?${params.toString()}`)
            .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
            .then(res => {
                if (!res.success) throw new Error(res.error);
    ...
    const generateConclusion = () => {
        ...
        if (topMedicines && topMedicines.length > 0) {
            ...
            summary += `<li><i class="bi bi-star-fill text-warning me-2"></i>Top Items: <strong>${names}</strong> are in highest demand this period &mdash; worth prioritizing when restocking.</li>`;
        }
        ...
        if (lowStock) {
            ...
            if (lowStockModel && lowStockModel.model === 'consumption_based_stock_depletion') {
                summary += `<li class="mb-2"><i class="bi bi-calculator text-info me-2"></i>This low-stock forecast uses a <strong>consumption-based depletion model</strong>, estimating how many days current stock will last based on average daily demand from the last <strong>${lowStockModel.lookbackDays ?? 30}</strong> days.</li>`;
            }
            ...
            if (crit.length === 0 && warn.length === 0) {
                summary += `<li><i class="bi bi-check-circle-fill text-success me-2"></i>All items are above both stock thresholds and not projected to run out within 14 days (based on the selected forecast period).</li>`;
            }
        }
    ...
    const loadAll = () => {
        ...
        if (!resolved.start || !resolved.end || new Date(resolved.start) > new Date(resolved.end)) {
            ['demandChart', 'topMedicinesChart', 'trendsChart', 'stockForecastChart'].forEach(id => {
                showEmptyState(id, 'Choose a date range, then click Apply');
            });
