document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadSales() {
        ...
        const url = '../Admin/api/sales.php' + (params.toString() ? '?' + params.toString() : '');
    ...
    function loadMedicines(selectElement, searchTerm = '') {
        ...
        searchTimeout = setTimeout(() => {
            const url = searchTerm 
                ? `../Admin/api/sales.php?action=get_medicines&search=${encodeURIComponent(searchTerm)}`
                : '../Admin/api/sales.php?action=get_medicines';
    ...
    document.getElementById('create-sale-form').addEventListener('submit', (e) => {
        ...
        fetch('../Admin/api/sales.php', { method: 'POST', body: formData })
            .then(response => response.text())
            .then(text => {
                try {
                    const data = safeJsonParse(text);
    ...
    tbody.addEventListener('click', (e) => {
        ...
        if (viewBtn) {
            ...
            fetch(`../Admin/api/sales.php?action=get_invoice&invoice_id=${invoiceId}`)
                .then(response => response.text())
                .then(text => {
                    try {
                            const data = safeJsonParse(text);
    ...
    let purchasesById = {};
    fetch('../Admin/api/sales.php?action=get_purchases')
        .then(response => response.text())
        .then(text => {
            try {
                const data = safeJsonParse(text);
