document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadMedicineCategories() {
        fetch('api/medicines.php?limit=1000&page=1')
            .then(r => r.json())
            .then(data => {
    ...
    function loadMedicines(page = 1) {
        ...
        let url = `api/medicines.php?search=${encodeURIComponent(search)}&sort=${sort}&page=${page}&limit=${itemsPerPage}&item_type=${item_type}`;
    ...
    addForm.addEventListener('submit', function (e) {
        ...
        fetch('api/medicines.php', { method: 'POST', body: formData })
            .then(r => r.json())
            .then(data => {
    ...
    tableBody.addEventListener('click', function (e) {
        ...
        if (btn.classList.contains('edit-btn')) {
            fetch(`api/medicines.php?id=${btn.dataset.id}`)
                .then(r => r.json())
                .then(data => {
                    if (data && !data.status) {
    ...
    confirmDeleteBtn?.addEventListener('click', () => {
        ...
        fetch(`api/medicines.php?id=${pendingDeleteId}`, { method: 'DELETE' })
            .then(r => r.json())
            .then(data => {
                if (data.status === 'success') {
                    deleteModal?.hide();
