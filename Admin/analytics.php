...
<body>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const loadLowStock = (s, e) => {
            fetch(`api/analytics.php?action=low_stock_forecast&start=${encodeURIComponent(s)}&end=${encodeURIComponent(e)}`)
                .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
                .then(res => {
                    if (!res.success) throw new Error(res.error);
        ...
        const loadModelForecasts = (s, e) => {
            fetch(`api/analytics.php?action=model_forecasts&start=${s}&end=${e}`)
                .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
                .then(res => {
                    if (!res.success) throw new Error(res.error || 'API error');
