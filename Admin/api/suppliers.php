...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'toggle_preferred') {
        ...
        $preferred = intval($_POST['preferred'] ?? 0) ? 1 : 0;

        if ($supplier_id <= 0) {
            $response['errors'] = ['Invalid supplier ID.'];
        } else {
            $stmt = $conn->prepare("UPDATE suppliers SET is_preferred = ? WHERE id = ?");
            if (!$stmt) {
                $response['errors'] = ['Prepare failed: ' . $conn->error];
            } else {
                $stmt->bind_param("ii", $preferred, $supplier_id);
                if ($stmt->execute()) {
                    $response['success'] = true;
                    $response['message'] = $preferred
                        ? 'Supplier marked as preferred.'
                        : 'Supplier removed from preferred suppliers.';
                } else {
                    $response['errors'] = ['Update failed: ' . $stmt->error];
                }
                $stmt->close();
    ...
    if ($action === 'delete') {
        $supplier_id = intval($_POST['id'] ?? 0);
        if ($supplier_id > 0) {
            // Check for related records first so we can give a specific, human
            // reason instead of surfacing a raw foreign-key error to the user.
            $blockers = [];
            $checks = [
                'orders'                 => ['label' => 'order(s) on record',              'col' => 'supplier_id'],
                'supplier_medicines'     => ['label' => 'item(s) linked to their catalog', 'col' => 'supplier_id'],
                'supplier_inventory'     => ['label' => 'inventory item(s) in stock',        'col' => 'supplier_id'],
                'supplier_sales'         => ['label' => 'sale/payout record(s)',             'col' => 'supplier_id'],
                'delivery_transactions'  => ['label' => 'delivery transaction(s)',           'col' => 'supplier_id'],
            ];
            foreach ($checks as $table => $info) {
                $chk = $conn->prepare("SELECT COUNT(*) AS c FROM `$table` WHERE `{$info['col']}` = ?");
                $chk->bind_param('i', $supplier_id);
                $chk->execute();
                $count = (int)$chk->get_result()->fetch_assoc()['c'];
                $chk->close();
                if ($count > 0) $blockers[] = "$count {$info['label']}";
            }

            if (!empty($blockers)) {
                $response['errors'] = [
                    'This supplier cannot be deleted right now because they still have ' .
                    implode(' and ', $blockers) .
                    '. Remove or reassign those first, or keep the supplier on file for your records instead of deleting.'
                ];
            } else {
            $conn->begin_transaction();
            try {
                ...
                $conn->rollback();
                // Fall back to a friendly message even if some other, unanticipated
                // constraint trips — never show the raw SQL error to the user.
                if (stripos($e->getMessage(), 'foreign key constraint') !== false) {
                    $response['errors'] = ['This supplier cannot be deleted right now because other records still reference them. Remove those first, or keep the supplier on file for your records instead of deleting.'];
                } else {
                    $response['errors'] = [$e->getMessage()];
                }
            }
    ...
    if ($action === 'add_medicine') {
        ...
        if ($supplier_id > 0 && $medicine_id > 0 && $unit_price >= 0) {
            ...
            try {
                ...
                $conn->commit();
                $response['success'] = true;
                $response['message'] = 'Item added to supplier successfully.';
            } catch (Exception $e) {
                $conn->rollback();
                $response['errors'] = [$e->getMessage()];
            }
    ...
    if ($action === 'edit_supplier_medicine') {
        ...
        if ($supplier_id <= 0 || $medicine_id <= 0) {
            $response['errors'] = ['Invalid supplier or item ID.'];
        } elseif ($unit_price < 0) {
            ...
            if (!$stmt) {
                ...
                if ($stmt->execute()) {
                    $response['success'] = true;
                    $response['message'] = 'Item supply details updated successfully.';
                } else {
                    $response['errors'] = ['Update failed: ' . $stmt->error];
                }
                $stmt->close();
    ...
    if ($action === 'delete_supplier_medicine') {
        ...
        if ($supplier_id > 0 && $medicine_id > 0) {
            ...
            if (!$stmt) {
                ...
                if ($stmt->execute()) {
                    $response['success'] = true;
                    $response['message'] = 'Item removed from supplier successfully.';
                } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && !isset($_GET['supplier_id'])) {
    ...
    $sql = "
        SELECT s.id, s.name, s.company, s.address, s.contact, s.is_preferred,
